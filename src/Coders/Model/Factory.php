...
class Factory
{
    ...
    public function create($schema, $table)
    {
        ...
        if ($model->indentWithSpace()) {
            $file = str_replace(
                "\t",
                str_repeat(' ', $model->indentWithSpace()),
                $file
            );
        }

        $this->files->put(
            $this->modelPath($model, $model->usesBaseFiles() ? ['Base'] : []),
            $file
        );
    ...
    public function referencing(Model $model)
    {
        ...
        foreach ($this->schemas as $schema) {
            $references = array_merge(
                $references,
                $schema->referencing($model->getBlueprint())
            );
        }
        ...
        foreach ($references as &$related) {
            ...
            $related['model'] = $model
                ->getBlueprint()
                ->is($blueprint->schema(), $blueprint->table())
                ? $model
                : $this->makeModel(
                    $blueprint->schema(),
                    $blueprint->table(),
    ...
    protected function fillTemplate($template, Model $model)
    {
        ...
        $dependencies = $this->shortenAndExtractImportableDependencies(
            $properties,
            $model
        );
        ...
        $dependencies = array_merge(
            $dependencies,
            $this->shortenAndExtractImportableDependencies($parentClass, $model)
        );
        ...
        $dependencies = array_merge(
            $dependencies,
            $this->shortenAndExtractImportableDependencies($body, $model)
        );
    ...
    private function imports($dependencies, Model $model)
    {
        ...
        foreach ($dependencies as $dependencyClass) {
            ...
            if (
                trim($dependencyClass, '\\') ==
                trim($model->getQualifiedUserClassName(), '\\')
            ) {
                continue;
            }

            // Do not import classes from same namespace
            $inCurrentNamespacePattern = str_replace(
                '\\',
                '\\\\',
                "/{$model->getBaseNamespace()}\\[a-zA-Z0-9_]*/"
            );
     ...
     */
    private function shortenAndExtractImportableDependencies(
        &$placeholder,
        $model
    ) {
        ...
        if (preg_match_all($qualifiedClassesPattern, $placeholder, $matches)) {
            foreach ($matches[1] as $usedClass) {
                ...
                if (
                    $model->usesBaseFiles() &&
                    $usedClass === $model->getQualifiedUserClassName()
                ) {
                    continue;
                }
                ...
                if (
                    ...
                    trim(implode('\\', $namespacePieces), '\\') !=
                        trim($model->getNamespace(), '\\')
                ) {
                    continue;
                }
    ...
    protected function properties(Model $model)
    {
        ...
        foreach ($model->getProperties() as $name => $hint) {
            $annotations .= $this->class->annotation(
                'property',
                "$hint \$$name"
            );
        }
    ...
    protected function body(Model $model)
    {
        ...
        if ($model->doesNotAutoincrement()) {
            $body .= $this->class->field('incrementing', false, [
                'visibility' => 'public',
            ]);
        }
        ...
        if (! $model->usesTimestamps()) {
            $body .= $this->class->field('timestamps', false, [
                'visibility' => 'public',
            ]);
        }
        ...
        if ($model->doesNotUseSnakeAttributes()) {
            $body .= $this->class->field('snakeAttributes', false, [
        ...
        if (
            $model->hasHidden() &&
            ($model->doesNotUseBaseFiles() || $model->hiddenInBaseFiles())
        ) {
        ...
        if (
            $model->hasFillable() &&
            ($model->doesNotUseBaseFiles() || $model->fillableInBaseFiles())
        ) {
        ...
        foreach ($model->getRelations() as $constraint) {
            $body .= $this->class->method(
                ...
                [
                    'before' => "\n",
                    'returnType' => $model->definesReturnTypes()
    ...
    {
        return ! $this->files->exists($this->modelPath($model)) &&
            $model->usesBaseFiles();
    }
    ...
    protected function createUserFile(Model $model)
    {
        ...
        $template = str_replace(
            '{{imports}}',
            $this->formatBaseClasses($model),
            $template
        );
        $template = str_replace(
            '{{parent}}',
            $this->getBaseClassName($model),
            $template
        );
        $template = str_replace(
            '{{body}}',
            $this->userFileBody($model),
            $template
        );
